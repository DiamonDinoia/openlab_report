...
To exploit the parallelism provided by these devices a channel level parallelization is performed but, a dynamic parallelism inside the channel it to be studied.%More precisely all the readout from the channel are buffered and then there is a kernel invocation on all of them.
